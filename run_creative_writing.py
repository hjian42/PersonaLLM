...
def main():
    ...
    parser.add_argument("--input_folder", default="./outputs/bfi/temp0.7", type=str)
    parser.add_argument("--output_folder", default="./outputs/bfi/temp0.7/writings", type=str)
    ...
    for run_id in range(1, 6):
        ...
        for persona_type in tqdm(list(itertools.product(*personality_types))):
            ...
            json_filepath = os.path.join(run_folder_path, "{}.json".format(persona_encoding))
            json_output = write_bfi_story(json_filepath, prompt_file=args.prompt_file)
            # print(json_output)
            # save results in json
            json_output = json.dumps(json_output, indent=4)
            with open(os.path.join(args.output_folder, "{}_{}.json".format(persona_encoding, run_folder)), "w") as out:
