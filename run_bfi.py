...
def fill_the_prompt(persona_type: list, prompt_file=None):
    ...
    prompt = prompt_template.replace("%PERSONA%", big_five_trait_str)
    prompt = prompt.strip("\n").strip()
    prompt = prompt + "\n\n"
    return prompt

def answer_bfi_questions(persona_type, prompt_file=None, output_folder=None):
    """"answer bfi questions for one persona"""
    filled_prompt = fill_the_prompt(persona_type, prompt_file)
...
def main():
    ...
    parser.add_argument("--output_folder", default="./outputs/bfi/temp0.7/run5", type=str)
    ...
    personality_types = [("extraverted", "introverted"),
    ...
    for persona_type in tqdm(list(itertools.product(*personality_types))):
        ...
        json_output = answer_bfi_questions(persona_type, prompt_file=args.prompt_file)
        
        # save results in json
        if json_output['text']:
            json_output = json.dumps(json_output, indent=4)
            persona_encoding = "_".join([trait[:3] for trait in persona_type])
            with open(os.path.join(args.output_folder, "{}.json".format(persona_encoding)), "w") as out:
